...
# Real work lives in scripts/; this file is a thin wrapper.
...
# Do not set .DEFAULT_GOAL here — the host Makefile owns the default target.
...
ruleset-help: ## ruleset 用ターゲットの詳細ヘルプを表示
	@printf '%s\n' \
		'Targets:' \
		'' \
...
ruleset-create: ## GitHub リポジトリを作成して ruleset を適用 (RULESET_REPO=OWNER/NAME)
...
ruleset-apply: ## 既存リポジトリに ruleset を適用/更新 (RULESET_REPO=OWNER/NAME)
	@if [[ -z "$(RULESET_REPO)" ]]; then \
		echo "error: RULESET_REPO=OWNER/NAME is required" >&2; \
...
ruleset-check: ## ruleset の一覧とブランチ適用状況を確認 (RULESET_REPO=OWNER/NAME)
	@if [[ -z "$(RULESET_REPO)" ]]; then \
		echo "error: RULESET_REPO=OWNER/NAME is required" >&2; \
